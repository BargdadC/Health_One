...
def contents(survey_id="5c66f3e38d341ffe76142f65"): # sur_str 를 survey_id 로 객체로 변경
    ...
    try:
        ...
        for pagerank_doc in db.pagerank.find():
            ...
            if flag:
                knowledge_dict = {}
                knowledge_dict['id'] = str(pagerank_doc['_id'])
                ...
                knowledge_dict['category'] = base64_encoding(pagerank_doc['category'])
                knowledge_dict['title'] = base64_encoding(pagerank_doc['title'])
                knowledge_dict['body'] = base64_encoding(pagerank_doc['body'])
                knowledge_dict['scope_list'] = pagerank_doc['scope_list']
        ...
        for pagerank_doc in db.pagerank.find():
            ...
            if flag:
                video_dict = {}
                video_dict['id'] = str(pagerank_doc['_id'])
                # video_dict['category'] = base64_encoding(video_doc['category'])
                # video_dict['keyword_title'] = base64_encoding(video_doc['keyword_title'])
                video_dict['category'] = base64_encoding(pagerank_doc['category'])
                video_dict['title'] = base64_encoding(pagerank_doc['title'])
                video_dict['body'] = base64_encoding(pagerank_doc['body'])
                video_dict['scope_list'] = pagerank_doc['scope_list']
