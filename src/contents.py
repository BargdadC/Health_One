...
def contents(survey_id="5c66f3e38d341ffe76142f65"): # sur_str 를 survey_id 로 객체로 변경
    ...
    try:
        ...
        for knowledge_doc in db.knowledge.find():
            ...
            # 문진결과와 복수조건 비교
            for scope_value in knowledge_doc['scope_list']:
                scope = convert_to_dict(scope_value)
                # print(scope)
                operator = str(scope['operator'])
                operand = scope['operand']
            ...
            if flag:
                knowledge_dict = {}
                knowledge_dict['id'] = str(knowledge_doc['_id'])
                ...
                knowledge_dict['category'] = base64_encoding(knowledge_doc['category'])
                knowledge_dict['title'] = base64_encoding(knowledge_doc['title'])
                knowledge_dict['body'] = base64_encoding(knowledge_doc['body'])
                knowledge_dict['scope_list'] = knowledge_doc['scope_list']
        ...
        # 동영상 구문 시작 
        #각 도큐먼트를 한개씩 조회
        for video_doc in db.video.find():
            flag = True;

            # 문진결과와 복수조건 비교
            for scope_value in video_doc['scope_list']:
                scope = convert_to_dict(scope_value)
                # print(scope)
                operator = str(scope['operator'])
                operand = scope['operand']
            ...
            if flag:
                video_dict = {}
                video_dict['id'] = str(video_doc['_id'])
                # video_dict['category'] = base64_encoding(video_doc['category'])
                # video_dict['keyword_title'] = base64_encoding(video_doc['keyword_title'])
                video_dict['category'] = base64_encoding(video_doc['category'])
                video_dict['title'] = base64_encoding(video_doc['title'])
                video_dict['body'] = base64_encoding(video_doc['body'])
                video_dict['scope_list'] = video_doc['scope_list']
