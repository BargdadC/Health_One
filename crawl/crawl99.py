            ...
            for pages in range(0, 6):
                ...
                for title, meta in zip(titles, meta_data):
                    try:
                        ...
                        blacklist = black_collection.find_one({"url":link})
                        if check == link:
                            continue
                        elif blacklist == link:
                            continue
                        elif check == None:
                            collection.insert({'title': title.text, 'url': link, 'date': date,
                                           'keyword_title_id': keyword_title_id, 'keyword_title':keyword_title, 'category':category, 'hit': 1,
