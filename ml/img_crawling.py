...
parser.add_argument("-enname", "--enpeople", required=True)
...
def main():
    ...
    #url 요청 파싱값
    enhtml_object = req.get(url_info,enparams) #html_object html source 값

    if (html_object.status_code == 200)and(enhtml_object.status_code == 200):
        #페이지 status_code 가 200 일때 2XX 는 성공을 이야기함
        bs_object = BeautifulSoup(html_object.text,"html.parser")
        enbs_object = BeautifulSoup(enhtml_object.text,"html.parser")
        #인스턴스 생성
        img_data = bs_object.find_all("img")
        enimg_data = enbs_object.find_all("img")
        ...
        for i in enumerate(img_data[1:]):
            ...
            filename = '/project/ml/test_photos/' + directory + '/'+str(i[0]+1)+'.jpg'
        ...
        for j in enumerate(enimg_data[1:]):
            #딕셔너리를 순서대로 넣어줌
            ent = urlopen(j[1].attrs['src']).read()


            filename = '/project/ml/test_photos/'+directory+'/'+str(j[0]+21)+'.jpg'


            with open(filename,"wb") as enf:
